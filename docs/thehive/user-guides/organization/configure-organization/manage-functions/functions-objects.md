...
* `caze.manageCaseAccess(idOrNumber: string, accessDetails: { users: string[], _kind: "UserAccessKind" } | { _kind: "OrganisationAccessKind" }): void`: Sets the access level for a case. Use `UserAccessKind` to restrict access to specific users, or `OrganisationAccessKind` to make the case public to the organization.
