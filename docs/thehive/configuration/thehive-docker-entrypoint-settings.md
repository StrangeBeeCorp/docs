...
|-----------|------|-------------|
| `--config-file <file>` | string | Specifies the path to the configuration file. In a Docker environment, this parameter is overridden by the configuration file generated by the entrypoint from environment variables. |
| `--no-config` | flag | Prevents TheHive from automatic configuration, including secrets and Elasticsearch settings. In a Docker environment, environment variables read by the entrypoint, such as `TH_CACERT`, aren't applied in this mode. |
